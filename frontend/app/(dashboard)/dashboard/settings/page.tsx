...
export default function SettingsPage() {
  ...
  return (
    <div className="page overflow-x-hidden">
      <header className="page-header mb-8 sm:mb-12">
        <div className="flex flex-col gap-2">
          <h1 className="text-4xl sm:text-6xl lg:text-[5rem] font-black tracking-tighter text-zinc-900 dark:text-zinc-50 leading-tight uppercase italic pointer-events-none select-none">
            Settings
          </h1>
      ...
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 items-start relative">
        ...
        {/* Tab Navigation */}
        <nav className="w-full lg:w-72 sticky top-20 z-10 flex lg:flex-col gap-2 overflow-x-auto lg:overflow-x-visible pb-4 lg:pb-0 scrollbar-hide">
          {tabs.map((tab) => {
            const Icon = tab.icon;
        ...
        <div className="flex-1 w-full min-w-0 animate-in fade-in slide-in-from-right-4 duration-500">
          <section className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] overflow-hidden shadow-2xl shadow-black/5 relative group">
            ...
            {/* AI Agent & Security Placeholders */}
            {(activeTab === "ai-agent" || activeTab === "security") && (
              <div className="h-96 flex flex-col items-center justify-center p-12 text-center animate-in zoom-in-95 duration-500">
                 <div className="w-16 h-16 bg-zinc-100 dark:bg-zinc-800 rounded-3xl flex items-center justify-center text-zinc-400 mb-6">
                    {activeTab === "ai-agent" ? <Bot size={32} /> : <Shield size={32} />}
                 </div>
                 <h4 className="text-xl font-black text-zinc-900 dark:text-white uppercase italic mb-2">Expansion Module Locked</h4>
                 <p className="text-sm font-bold text-zinc-500 max-w-xs">{activeTab === "ai-agent" ? "Configure your neural connection settings and context memory depth." : "Manage your identity tokens and session security protocols."}</p>
                 <div className="mt-8 px-5 py-2 bg-zinc-100 dark:bg-zinc-800 rounded-full text-[10px] font-black text-zinc-400 uppercase tracking-widest">
                    Available in v1.2
                 </div>
              </div>
            )}
